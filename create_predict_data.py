...
def split_number(input_file, segment_length_seconds=60):
    ...
    total_segments = len(y) // segment_length_samples + (
        1 if len(y) % segment_length_samples != 0 else 0)
...
def process_directory(input_dir, output_dir, target_sr=16000):
    ...
    # Get all audio files
    audio_files = [
        f for f in os.listdir(input_dir) if f.endswith(('.mp3', '.wav'))
    ]
...
def process_audio_files(input_file, output_dir, segment_length=60):
    ...
    resampled_dir = os.path.join(output_dir, 'process')
    ...
    audio_files = [
        f for f in os.listdir(resampled_dir) if f.endswith(('.mp3', '.wav'))
    ]
    ...
    feature_names = opensmile.Smile(
        ...
        feature_level=opensmile.FeatureLevel.Functionals,
        sampling_rate=16000).feature_names
